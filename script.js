...
const playlist = [
    { title: "Morning Side", src: "audio/Morningside.mp3" },
    { title: "Everlasting Truth", src: "audio/EverlastingTruth.mp3" },
    { title: "Golden Hour", src: "audio/GoldenHour.mp3" },
    { title: "Glow", src: "audio/glow.mp3" },
    { title: "Sunflowers", src: "audio/Sunflowers.mp3" },
    { title: "Worry", src: "audio/Worry.mp3" },
...
function handleAmbientMixer(slider, audioEl, readoutId) {
    ...
    // Ensure GitHub allows streaming cross-origin audio
    ...
    if (audioEl.id === 'fire-audio') {
        ...
        slider.addEventListener('input', (e) => {
            ...
            if (val == 0) {
                ...
                audioEl.loop = true; 
                if (audioEl.paused) {
                    audioEl.play().catch(o => console.log("Ambient playback blocked:", o));
                }
            }
        });
    }
} // <--- The function ends cleanly here!

// ==========================================
// INITIALIZE MIXERS (At the bottom of your file)
// ==========================================
...
// 3. Water Bubbles (Extracted out of the function so it works safely!)
const bubblesSlider = document.getElementById('bubbles-slider');
const bubblesAudio = document.getElementById('bubbles-audio');

if (bubblesSlider && bubblesAudio) {
    handleAmbientMixer(bubblesSlider, bubblesAudio, 'bubbles-readout');
}
...
const fortunes = [
    ...
    "Today is your lucky day! Seriously, go ride that wave. 🌊",
    "Something completely unexpected is going to make you smile today. 🎁",
    "A small piece of good news will find its way to you before the day ends. 🌟",
    "The universe is plotting to surprise you with something lovely today. ⏰",
    "You are going to find exactly what you've been looking for when you least expect it. 🔍",

    // --- Cozy & Comfort Vibes ---
    "A cozy space brings a clear mind. Take a deep breath and relax. 🌧️",
    "It's a perfect day to wrap yourself up, put on your favorite tracks, and just unwind. 🎧",
    "The warm drink you have today is going to taste absolutely perfect. ☕",
    "A quiet, peaceful moment is waiting for you later today. 🕯️",

    // --- Mindful & Uplifting ---
    "A great vibe attracts great people. Keep being exactly who you are. ✨",
    "You are doing much better than you give yourself credit for. 🌱",
    "Someone is thinking of you right now and deeply appreciates your presence in their life. ❤️"
];
...
function crackCookie() {
    ...
    const dailyFortune = fortunes[randomIndex];
    
    display.innerText = `🥠 Fortune: "${dailyFortune}"`;
    
    localStorage.setItem('cookieLastOpenedDate', today);
    localStorage.setItem('cookieLastFortune', dailyFortune);
    
    btn.disabled = true;
    btn.innerText = "🥠 Cookie Already Cracked Today";
}

function checkDailyCookieStatus() {
    const btn = document.getElementById('cookie-btn');
    const display = document.getElementById('cookie-display');
    ...
    const today = new Date().toDateString();
    const lastOpened = localStorage.getItem('cookieLastOpenedDate');
    const lastFortune = localStorage.getItem('cookieLastFortune');
    
    if (lastOpened === today) {
        btn.disabled = true;
        btn.innerText = "🥠 Cookie Already Cracked Today";
        display.innerText = `🥠 Today's Fortune: "${lastFortune}"`;
    }
}
